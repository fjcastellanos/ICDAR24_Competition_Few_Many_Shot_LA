...
def gen_mask(img):
    ...
    C, H, W = img.shape
    mask = np.zeros([H, W])
    main_text = np.where(np.logical_and(img[0, :, :] == 255, np.logical_and(img[1, :, :] == 0, img[2, :, :] == 255)))
    decoration = np.where(np.logical_and(img[0, :, :] == 0, np.logical_and(img[1, :, :] == 255, img[2, :, :] == 255)))
    title = np.where(np.logical_and(img[0, :, :] == 255, np.logical_and(img[1, :, :] == 0, img[2, :, :] == 0)))
    chapter_headings = np.where(np.logical_and(img[0, :, :] == 0, np.logical_and(img[1, :, :] == 255, img[2, :, :] == 0)))
    paratext = np.where(np.logical_and(img[0, :, :] == 255, np.logical_and(img[1, :, :] == 255, img[2, :, :] == 0)))
...
for DS in ["Latin2FS", "Latin14396FS", "Latin16746FS", "Syr341FS"]:
    ...
    res = udiads_evaluate(result_directory=f"dataset/U-DIADS-Bib-FS/{DS}/pixel-level-gt-Latin2FS/val_results", gt_directory=f"dataset/U-DIADS-Bib-FS/{DS}/pixel-level-gt-Latin2FS/validation")
    result.append(res)
